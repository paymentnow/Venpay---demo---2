...
def pay(ref):
    ...
    if bank:
        return redirect(f"/mobile_redirect/{ref}")
...
""", banks=bank_list, ref=ref)

@app.route("/set_bank/<ref>", methods=["POST"])
def set_bank(ref):
    url = request.form.get("bank_url")
    resp = make_response(redirect(f"/mobile_redirect/{ref}"))
...
def mobile_redirect(ref):
    ...
    if bank == "td":
        deeplink = f"intent://sendmoney?email={email}&amount={amount}#Intent;scheme=td;package=com.td;end"
    elif bank == "koho":
        deeplink = f"koho://etransfer/send?email={email}&amount={amount}"
    else:
...
<html>
<head>
    <title>DanielPay | Opening App</title>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <script>
    setTimeout(function() {{
